...
void MarketScenario::process_events() {
    ...
    while (!scheduled_events_.empty() && 
           scheduled_events_.front().scheduled_time <= now) {
...
void RecoveryScenario::simulate_recovery() {
    Logger::instance().log(LogLevel::INFO, "Simulating recovery - lost " + 
                          std::to_string(messages_lost_) + " messages");
...
std::string ScenarioManager::get_scenario_name(ScenarioType type) const {
    switch (type) {
        ...
        case ScenarioType::FastMarket:  return "fast";
        case ScenarioType::ThinMarket:  return "thin";
        case ScenarioType::Recovery:    return "recovery";
        case ScenarioType::Volatile:    return "volatile";
        case ScenarioType::Opening:     return "opening";
        case ScenarioType::Closing:     return "closing";
...
ScenarioType ScenarioManager::parse_scenario_type(const std::string& str) const {
    ...
    if (lower == "normal")     return ScenarioType::Normal;
    if (lower == "fast")       return ScenarioType::FastMarket;
    if (lower == "thin")       return ScenarioType::ThinMarket;
    if (lower == "recovery")   return ScenarioType::Recovery;
    if (lower == "volatile")   return ScenarioType::Volatile;
    if (lower == "opening")    return ScenarioType::Opening;
    if (lower == "closing")    return ScenarioType::Closing;
