...
uint32_t SBEDecoder::decode_uint32()
{
    if (!can_read(4))
        return 0;
    uint32_t value = buffer_[offset_] | (buffer_[offset_ + 1] << 8) | (buffer_[offset_ + 2] << 16) | (buffer_[offset_ + 3] << 24);
    offset_ += 4;
    return value;
}
