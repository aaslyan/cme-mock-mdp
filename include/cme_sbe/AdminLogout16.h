...
#ifndef _CME_SBE_ADMINLOGOUT16_CXX_H_
#define _CME_SBE_ADMINLOGOUT16_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(180);
    ...
    {
        AdminLogout16 skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #ifdef SBE_USE_SPAN
    SBE_NODISCARD std::span<const char> getTextAsSpan() const SBE_NOEXCEPT
    {
        const char *buffer = m_buffer + m_offset + 0;
        return std::span<const char>(reinterpret_cast<const char*>(buffer), 180);
    }
    #endif

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AdminLogout16 &putText(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 180, "array too large for putText");

        std::memcpy(m_buffer + m_offset + 0, src.data(), sizeof(char) * N);
        for (std::size_t start = N; start < 180; ++start)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    AdminLogout16 &putText(T&& src)  SBE_NOEXCEPT requires
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AdminLogout16 &putText(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putText(std::span<const char, N>(src));
    }
    #endif
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const AdminLogout16 &_writer)
{
