...
#ifndef _CME_SBE_SECURITYSTATUS30_CXX_H_
#define _CME_SBE_SECURITYSTATUS30_CXX_H_
...
public:
    ...
    {
        SecurityStatus30 skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SecurityStatus30 &putSecurityGroup(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    SecurityStatus30 &putSecurityGroup(T&& src)  SBE_NOEXCEPT requires
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    SecurityStatus30 &putSecurityGroup(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    SecurityStatus30 &putAsset(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 6, "array too large for putAsset");

        std::memcpy(m_buffer + m_offset + 14, src.data(), sizeof(char) * N);
        for (std::size_t start = N; start < 6; ++start)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    SecurityStatus30 &putAsset(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    SecurityStatus30 &putAsset(const char (&src)[N]) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SecurityStatus30 &_writer)
{
