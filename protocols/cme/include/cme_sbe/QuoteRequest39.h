...
#ifndef _CME_SBE_QUOTEREQUEST39_CXX_H_
#define _CME_SBE_QUOTEREQUEST39_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(35);
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(39);
    ...
    {
        QuoteRequest39 skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    #ifdef SBE_USE_SPAN
    SBE_NODISCARD std::span<const char> getQuoteReqIDAsSpan() const SBE_NOEXCEPT
    {
        const char *buffer = m_buffer + m_offset + 8;
        return std::span<const char>(reinterpret_cast<const char*>(buffer), 23);
    }
    #endif

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    QuoteRequest39 &putQuoteReqID(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 23, "array too large for putQuoteReqID");

        std::memcpy(m_buffer + m_offset + 8, src.data(), sizeof(char) * N);
        for (std::size_t start = N; start < 23; ++start)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    QuoteRequest39 &putQuoteReqID(T&& src)  SBE_NOEXCEPT requires
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    QuoteRequest39 &putQuoteReqID(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putQuoteReqID(std::span<const char, N>(src));
    }
    #endif
...
public:
    ...
    #pragma GCC diagnostic pop
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        NoRelatedSym &putSymbol(std::span<const char, N> src) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        NoRelatedSym &putSymbol(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        NoRelatedSym &putSymbol(const char (&src)[N]) SBE_NOEXCEPT
        {
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const QuoteRequest39 &_writer)
{
