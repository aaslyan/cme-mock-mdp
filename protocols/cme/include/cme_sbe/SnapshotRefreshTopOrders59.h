...
#ifndef _CME_SBE_SNAPSHOTREFRESHTOPORDERS59_CXX_H_
#define _CME_SBE_SNAPSHOTREFRESHTOPORDERS59_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(13);
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(59);
    ...
    {
        SnapshotRefreshTopOrders59 skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SnapshotRefreshTopOrders59 &_writer)
{
