...
#ifndef _LSEG_SBE_MARKETDATASNAPSHOTFULLREFRESH_CXX_H_
#define _LSEG_SBE_MARKETDATASNAPSHOTFULLREFRESH_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(55);
    ...
    MarketDataSnapshotFullRefresh(char *buffer, const std::uint64_t bufferLength) :
        MarketDataSnapshotFullRefresh(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MarketDataSnapshotFullRefresh(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MarketDataSnapshotFullRefresh(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MarketDataSnapshotFullRefresh &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    MarketDataSnapshotFullRefresh &sbeRewind()
    {
    ...
    {
        MarketDataSnapshotFullRefresh skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    MarketDataSnapshotFullRefresh &applID(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
    ...
    MarketDataSnapshotFullRefresh &putApplID(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    MarketDataSnapshotFullRefresh &putApplID(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    MarketDataSnapshotFullRefresh &putApplID(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    MarketDataSnapshotFullRefresh &putApplID(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    MarketDataSnapshotFullRefresh &putApplID(const std::string_view str)
    {
    ...
    #else
    MarketDataSnapshotFullRefresh &putApplID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    MarketDataSnapshotFullRefresh &bookStateID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    MarketDataSnapshotFullRefresh &conflationID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_BIG_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD BooleanEnum::Value lastFragment() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 24, sizeof(std::uint8_t));
    ...
    MarketDataSnapshotFullRefresh &lastFragment(const BooleanEnum::Value value) SBE_NOEXCEPT
    {
    ...
    MarketDataSnapshotFullRefresh &securityID(const std::uint64_t index, const char value)
    {
        if (index >= 10)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    MarketDataSnapshotFullRefresh &putSecurityID(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 10, "array too large for putSecurityID");

        std::memcpy(m_buffer + m_offset + 25, src.data(), sizeof(char) * N);
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    MarketDataSnapshotFullRefresh &putSecurityID(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    MarketDataSnapshotFullRefresh &putSecurityID(const char *const src) SBE_NOEXCEPT
    #endif
    {
        std::memcpy(m_buffer + m_offset + 25, src, sizeof(char) * 10);
        return *this;
    }

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    MarketDataSnapshotFullRefresh &putSecurityID(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    MarketDataSnapshotFullRefresh &putSecurityID(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 10)
        {
    ...
    MarketDataSnapshotFullRefresh &mDSubBookType(const std::uint8_t value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    MarketDataSnapshotFullRefresh &securityType(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    MarketDataSnapshotFullRefresh &putSecurityType(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 8, "array too large for putSecurityType");

        std::memcpy(m_buffer + m_offset + 36, src.data(), sizeof(char) * N);
        for (std::size_t start = N; start < 8; ++start)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    MarketDataSnapshotFullRefresh &putSecurityType(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    MarketDataSnapshotFullRefresh &putSecurityType(const char *const src) SBE_NOEXCEPT
    #endif
    {
        std::memcpy(m_buffer + m_offset + 36, src, sizeof(char) * 8);
        return *this;
    }

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    MarketDataSnapshotFullRefresh &putSecurityType(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    MarketDataSnapshotFullRefresh &putSecurityType(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    #else
    MarketDataSnapshotFullRefresh &putSecurityType(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    MarketDataSnapshotFullRefresh &mDSecurityTradingStatus(const SecurityTradingStatusEnum::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    MarketDataSnapshotFullRefresh &mDHaltReason(const HaltReasonEnum::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    {
        std::uint64_t val;
        std::memcpy(&val, m_buffer + m_offset + 46, sizeof(std::uint64_t));
        return SBE_BIG_ENDIAN_ENCODE_64(val);
    }

    MarketDataSnapshotFullRefresh &creditStateID(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_BIG_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 46, &val, sizeof(std::uint64_t));
        return *this;
    }
    ...
    MarketDataSnapshotFullRefresh &messageEventSource(const MessageEventSourceEnum::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    #pragma GCC diagnostic pop
            ...
            m_bufferLength = bufferLength;
            GroupSizeEncoding dimensions(buffer, *pos, bufferLength, actingVersion);
            dimensions.blockLength(static_cast<std::uint16_t>(76));
        ...
        MDEntries &mDEntryType(const EntryTypeEnum::Value value) SBE_NOEXCEPT
        {
            char val = (value);
        ...
        MDEntries &mDEntryID(const std::uint64_t index, const char value)
        {
            if (index >= 20)
            {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        MDEntries &putMDEntryID(std::span<const char, N> src) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        MDEntries &putMDEntryID(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
        MDEntries &putMDEntryID(const char *const src) SBE_NOEXCEPT
        #endif
        {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        MDEntries &putMDEntryID(const char (&src)[N]) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_STRING_VIEW
        MDEntries &putMDEntryID(const std::string_view str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 20)
            {
        ...
        #else
        MDEntries &putMDEntryID(const std::string &str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 20)
            {
...
public:
        ...
        {
            m_mDEntryPx.wrap(m_buffer, m_offset + 21, m_actingVersion, m_bufferLength);
            return m_mDEntryPx;
        }
...
public:
        ...
        {
            m_mDEntrySize.wrap(m_buffer, m_offset + 29, m_actingVersion, m_bufferLength);
...
public:
        ...
        {
            m_mDEntryTime.wrap(m_buffer, m_offset + 37, m_actingVersion, m_bufferLength);
        ...
        MDEntries &numberOfOrders(const std::uint32_t value) SBE_NOEXCEPT
        {
        ...
        {
            std::uint32_t val;
            std::memcpy(&val, m_buffer + m_offset + 49, sizeof(std::uint32_t));
            return SBE_BIG_ENDIAN_ENCODE_32(val);
        }

        MDEntries &mDEntryPositionNo(const std::uint32_t value) SBE_NOEXCEPT
        {
            std::uint32_t val = SBE_BIG_ENDIAN_ENCODE_32(value);
            std::memcpy(m_buffer + m_offset + 49, &val, sizeof(std::uint32_t));
            return *this;
        }
        ...
        {
            std::uint32_t val;
            std::memcpy(&val, m_buffer + m_offset + 53, sizeof(std::uint32_t));
            return SBE_BIG_ENDIAN_ENCODE_32(val);
        }

        MDEntries &mDPriceLevel(const std::uint32_t value) SBE_NOEXCEPT
        {
            std::uint32_t val = SBE_BIG_ENDIAN_ENCODE_32(value);
            std::memcpy(m_buffer + m_offset + 53, &val, sizeof(std::uint32_t));
            return *this;
        }
        ...
        MDEntries &aggressorSide(const AggressorSideEnum::Value value) SBE_NOEXCEPT
        {
            char val = (value);
        ...
        MDEntries &screenedMDType(const ScreenedMDRequestTypeEnum::Value value) SBE_NOEXCEPT
        {
            std::uint8_t val = (value);
        ...
        MDEntries &ordPFill(const BooleanEnum::Value value) SBE_NOEXCEPT
        {
            std::uint8_t val = (value);
        ...
        MDEntries &mDEntryCalculationPeriod(const std::int32_t value) SBE_NOEXCEPT
        {
...
public:
        ...
        {
            m_mDEntryClosingTime.wrap(m_buffer, m_offset + 64, m_actingVersion, m_bufferLength);
        ...
        MDEntries &creditCode(const std::uint64_t index, const char value)
        {
            if (index >= 4)
            {
        ...
        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        MDEntries &putCreditCode(std::span<const char, N> src) SBE_NOEXCEPT
        {
        ...
        #ifdef SBE_USE_SPAN
        template <typename T>
        MDEntries &putCreditCode(T&& src)  SBE_NOEXCEPT requires
            (std::is_pointer_v<std::remove_reference_t<T>> &&
             !std::is_array_v<std::remove_reference_t<T>>)
        #else
        MDEntries &putCreditCode(const char *const src) SBE_NOEXCEPT
        #endif
        {
            std::memcpy(m_buffer + m_offset + 72, src, sizeof(char) * 4);
            return *this;
        }

        #ifdef SBE_USE_SPAN
        template <std::size_t N>
        MDEntries &putCreditCode(const char (&src)[N]) SBE_NOEXCEPT
        {
            return putCreditCode(std::span<const char, N>(src));
        }
        #endif

        MDEntries &putCreditCode(
            const char value0,
            const char value1,
            const char value2,
        ...
        {
            char val0 = (value0);
            std::memcpy(m_buffer + m_offset + 72, &val0, sizeof(char));
            char val1 = (value1);
            std::memcpy(m_buffer + m_offset + 73, &val1, sizeof(char));
            char val2 = (value2);
            std::memcpy(m_buffer + m_offset + 74, &val2, sizeof(char));
            char val3 = (value3);
            std::memcpy(m_buffer + m_offset + 75, &val3, sizeof(char));
        ...
        #ifdef SBE_USE_STRING_VIEW
        MDEntries &putCreditCode(const std::string_view str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 4)
            {
        ...
        #else
        MDEntries &putCreditCode(const std::string &str)
        {
            const std::size_t srcLength = str.length();
            if (srcLength > 4)
            {
...
public:
        ...
        template<typename CharT, typename Traits>
        friend std::basic_ostream<CharT, Traits> & operator << (
            std::basic_ostream<CharT, Traits> &builder, MDEntries &writer)
        {
            builder << '{';
...
private:
    MDEntries m_mDEntries;

public:
    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t MDEntriesId() SBE_NOEXCEPT
    {
        return 35005;
    }

    SBE_NODISCARD inline MDEntries &mDEntries()
    {
        m_mDEntries.wrapForDecode(m_buffer, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_mDEntries;
    }

    MDEntries &mDEntriesCount(const std::uint16_t count)
    {
        m_mDEntries.wrapForEncode(m_buffer, count, sbePositionPtr(), m_actingVersion, m_bufferLength);
        return m_mDEntries;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t mDEntriesSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool mDEntriesInActingVersion() const SBE_NOEXCEPT
    {
        return true;
    }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MarketDataSnapshotFullRefresh &_writer)
{
    MarketDataSnapshotFullRefresh writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MarketDataSnapshotFullRefresh", )";
...
void skip()
{
    auto &mDEntriesGroup { mDEntries() };
    while (mDEntriesGroup.hasNext())
    {
        mDEntriesGroup.next().skip();
    }
}
...
SBE_NODISCARD static std::size_t computeLength(const std::vector<std::tuple<std::size_t>> &mDEntriesItemLengths = {})
{
#if defined(__GNUG__) && !defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += MDEntries::sbeHeaderSize();
    if (mDEntriesItemLengths.size() > 65534LL)
    {
        throw std::runtime_error("mDEntriesItemLengths.size() outside of allowed range [E110]");
    }

    for (const auto &e: mDEntriesItemLengths)
    {
        #if __cplusplus >= 201703L
        length += std::apply(MDEntries::computeLength, e);
        #else
        length += MDEntries::computeLength(std::get<0>(e));
        #endif
    }
