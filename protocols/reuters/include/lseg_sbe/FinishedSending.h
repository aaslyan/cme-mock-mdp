...
#ifndef _LSEG_SBE_FINISHEDSENDING_CXX_H_
#define _LSEG_SBE_FINISHEDSENDING_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(29);
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(512);
    ...
    FinishedSending(char *buffer, const std::uint64_t bufferLength) :
        FinishedSending(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    FinishedSending(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        FinishedSending(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    FinishedSending &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    FinishedSending &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        FinishedSending skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    FinishedSending &messageType(const MessageTypeEnum::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    FinishedSending &sessionID(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    FinishedSending &putSessionID(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    FinishedSending &putSessionID(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    FinishedSending &putSessionID(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    FinishedSending &putSessionID(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    FinishedSending &putSessionID(const std::string_view str)
    {
    ...
    #else
    FinishedSending &putSessionID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    SBE_NODISCARD static const char *LastSeqNoMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t lastSeqNoId() SBE_NOEXCEPT
    {
        return 1181;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t lastSeqNoSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool lastSeqNoInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t lastSeqNoEncodingOffset() SBE_NOEXCEPT
    {
        return 21;
    }

    static SBE_CONSTEXPR std::uint64_t lastSeqNoNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_UINT64;
    }

    static SBE_CONSTEXPR std::uint64_t lastSeqNoMinValue() SBE_NOEXCEPT
    {
        return UINT64_C(0x0);
    }

    static SBE_CONSTEXPR std::uint64_t lastSeqNoMaxValue() SBE_NOEXCEPT
    {
        return UINT64_C(0xfffffffffffffffe);
    }

    static SBE_CONSTEXPR std::size_t lastSeqNoEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::uint64_t lastSeqNo() const SBE_NOEXCEPT
    {
        std::uint64_t val;
    ...
    FinishedSending &lastSeqNo(const std::uint64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const FinishedSending &_writer)
{
    FinishedSending writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "FinishedSending", )";
