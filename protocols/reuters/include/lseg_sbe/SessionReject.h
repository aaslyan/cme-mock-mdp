...
#ifndef _LSEG_SBE_SESSIONREJECT_CXX_H_
#define _LSEG_SBE_SESSIONREJECT_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(502);
    ...
    SessionReject(char *buffer, const std::uint64_t bufferLength) :
        SessionReject(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SessionReject(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SessionReject(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SessionReject &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SessionReject &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SessionReject skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    SessionReject &messageType(const MessageTypeEnum::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    SessionReject &sessionID(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    SessionReject &putSessionID(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    SessionReject &putSessionID(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    SessionReject &putSessionID(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    SessionReject &putSessionID(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    SessionReject &putSessionID(const std::string_view str)
    {
    ...
    #else
    SessionReject &putSessionID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
...
public:
    ...
    SBE_NODISCARD static const char *RejectCodeMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t rejectCodeId() SBE_NOEXCEPT
    {
        return 40005;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t rejectCodeSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool rejectCodeInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t rejectCodeEncodingOffset() SBE_NOEXCEPT
    {
        return 29;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t rejectCodeEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD char rejectCodeRaw() const SBE_NOEXCEPT
    {
        char val;
        std::memcpy(&val, m_buffer + m_offset + 29, sizeof(char));
        return (val);
    }

    SBE_NODISCARD SessionRejectCodeEnum::Value rejectCode() const
    {
        char val;
        std::memcpy(&val, m_buffer + m_offset + 29, sizeof(char));
        return SessionRejectCodeEnum::get((val));
    }

    SessionReject &rejectCode(const SessionRejectCodeEnum::Value value) SBE_NOEXCEPT
    {
        char val = (value);
        std::memcpy(m_buffer + m_offset + 29, &val, sizeof(char));
        return *this;
    }
    ...
    static const char *reasonCharacterEncoding() SBE_NOEXCEPT
    {
        return "UTF-8";
    }

    static SBE_CONSTEXPR std::uint64_t reasonSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    bool reasonInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }
    ...
    static SBE_CONSTEXPR std::uint64_t reasonHeaderLength() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD std::uint16_t reasonLength() const
    {
        std::uint16_t length;
    ...
    std::uint64_t getReason(char *dst, const std::uint64_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    SessionReject &putReason(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    SessionReject &putReason(const std::string &str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
        }
        return putReason(str.data(), static_cast<std::uint16_t>(str.length()));
    }

    #if __cplusplus >= 201703L
    SessionReject &putReason(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SessionReject &_writer)
{
    SessionReject writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    builder << R"("RejectCode": )";
    builder << '"' << writer.rejectCode() << '"';
