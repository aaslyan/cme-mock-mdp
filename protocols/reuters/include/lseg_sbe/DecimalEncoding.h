...
#ifndef _LSEG_SBE_DECIMALENCODING_CXX_H_
#define _LSEG_SBE_DECIMALENCODING_CXX_H_
...
public:
    ...
    DecimalEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        DecimalEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    DecimalEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        DecimalEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    DecimalEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    DecimalEncoding &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, DecimalEncoding &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
