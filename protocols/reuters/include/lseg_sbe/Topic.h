...
#ifndef _LSEG_SBE_TOPIC_CXX_H_
#define _LSEG_SBE_TOPIC_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(510);
    ...
    Topic(char *buffer, const std::uint64_t bufferLength) :
        Topic(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    Topic(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        Topic(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    Topic &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    Topic &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        Topic skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Topic &messageType(const MessageTypeEnum::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    Topic &sessionID(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    Topic &putSessionID(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    Topic &putSessionID(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    Topic &putSessionID(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    Topic &putSessionID(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    Topic &putSessionID(const std::string_view str)
    {
    ...
    #else
    Topic &putSessionID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    SBE_NODISCARD static const char *FlowMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t flowId() SBE_NOEXCEPT
    {
        return 40004;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t flowSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool flowInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t flowEncodingOffset() SBE_NOEXCEPT
    {
        return 21;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t flowEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD char flowRaw() const SBE_NOEXCEPT
    {
        char val;
    ...
    SBE_NODISCARD FlowTypeEnum::Value flow() const
    {
        char val;
        std::memcpy(&val, m_buffer + m_offset + 21, sizeof(char));
        return FlowTypeEnum::get((val));
    }

    Topic &flow(const FlowTypeEnum::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    SBE_NODISCARD static const char *ClassificationMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t classificationId() SBE_NOEXCEPT
    {
        return 40014;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t classificationSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool classificationInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t classificationEncodingOffset() SBE_NOEXCEPT
    {
        return 22;
    }
...
public:
    SBE_NODISCARD Classification &classification()
    {
        m_classification.wrap(m_buffer, m_offset + 22, m_actingVersion, m_bufferLength);
        return m_classification;
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Topic &_writer)
{
    Topic writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    builder << R"("Classification": )";
    builder << writer.classification();
