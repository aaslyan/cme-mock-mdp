...
#ifndef _LSEG_SBE_TERMINATE_CXX_H_
#define _LSEG_SBE_TERMINATE_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(503);
    ...
    {
        Terminate skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Terminate &messageType(const MessageTypeEnum::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    Terminate &sessionID(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    Terminate &putSessionID(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    Terminate &putSessionID(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    Terminate &putSessionID(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    Terminate &putSessionID(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    Terminate &putSessionID(const std::string_view str)
    {
    ...
    #else
    Terminate &putSessionID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
    ...
    SBE_NODISCARD char terminationCodeRaw() const SBE_NOEXCEPT
    {
        char val;
    ...
    SBE_NODISCARD TerminationCodeEnum::Value terminationCode() const
    {
        char val;
        std::memcpy(&val, m_buffer + m_offset + 21, sizeof(char));
        return TerminationCodeEnum::get((val));
    }

    Terminate &terminationCode(const TerminationCodeEnum::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    Terminate &putReason(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    Terminate &putReason(const std::string &str)
    {
        if (str.length() > 65534)
        {
    ...
    #if __cplusplus >= 201703L
    Terminate &putReason(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Terminate &_writer)
{
    Terminate writer(
        _writer.m_buffer,
        _writer.m_offset,
