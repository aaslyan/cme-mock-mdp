...
#ifndef _LSEG_SBE_MARKETDATAREQUESTREJECTION_CXX_H_
#define _LSEG_SBE_MARKETDATAREQUESTREJECTION_CXX_H_
...
class MarketDataRequestRejection
{
private:
...
public:
    ...
    MarketDataRequestRejection() = default;

    MarketDataRequestRejection(
        char *buffer,
    ...
    MarketDataRequestRejection(char *buffer, const std::uint64_t bufferLength) :
        MarketDataRequestRejection(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MarketDataRequestRejection(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MarketDataRequestRejection(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MarketDataRequestRejection &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MarketDataRequestRejection &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    MarketDataRequestRejection &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    MarketDataRequestRejection &sbeRewind()
    {
    ...
    {
        MarketDataRequestRejection skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    MarketDataRequestRejection &mDReqID(const std::uint64_t index, const char value)
    {
        if (index >= 8)
        {
    ...
    MarketDataRequestRejection &putMDReqID(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    MarketDataRequestRejection &putMDReqID(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    MarketDataRequestRejection &putMDReqID(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    MarketDataRequestRejection &putMDReqID(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    MarketDataRequestRejection &putMDReqID(const std::string_view str)
    {
    ...
    #else
    MarketDataRequestRejection &putMDReqID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 8)
        {
    ...
    SBE_NODISCARD static const char *MDReqRejReasonMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::SEMANTIC_TYPE: return "String";
    ...
    static const char *mDReqRejReasonCharacterEncoding() SBE_NOEXCEPT
    {
        return "UTF-8";
    }
    ...
    static SBE_CONSTEXPR std::uint64_t mDReqRejReasonHeaderLength() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD std::uint16_t mDReqRejReasonLength() const
    {
        std::uint16_t length;
    ...
    std::uint64_t skipMDReqRejReason()
    {
    ...
    SBE_NODISCARD const char *mDReqRejReason()
    {
        std::uint16_t lengthFieldValue;
    ...
    std::uint64_t getMDReqRejReason(char *dst, const std::uint64_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    MarketDataRequestRejection &putMDReqRejReason(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    std::string getMDReqRejReasonAsString()
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    std::string getMDReqRejReasonAsJsonEscapedString()
    {
        std::ostringstream oss;
        std::string s = getMDReqRejReasonAsString();
    ...
    #if __cplusplus >= 201703L
    std::string_view getMDReqRejReasonAsStringView()
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    MarketDataRequestRejection &putMDReqRejReason(const std::string &str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
        }
        return putMDReqRejReason(str.data(), static_cast<std::uint16_t>(str.length()));
    }

    #if __cplusplus >= 201703L
    MarketDataRequestRejection &putMDReqRejReason(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MarketDataRequestRejection &_writer)
{
    MarketDataRequestRejection writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MarketDataRequestRejection", )";
    ...
    builder << ", ";
    builder << R"("MDReqRejReason": )";
    builder << '"' <<
        writer.getMDReqRejReasonAsJsonEscapedString().c_str() << '"';
...
SBE_NODISCARD static std::size_t computeLength(std::size_t mDReqRejReasonLength = 0)
{
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += mDReqRejReasonHeaderLength();
    if (mDReqRejReasonLength > 65534LL)
    {
        throw std::runtime_error("mDReqRejReasonLength too long for length type [E109]");
    }
    length += mDReqRejReasonLength;
