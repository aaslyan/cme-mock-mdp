...
#ifndef _LSEG_SBE_QTYENCODING_CXX_H_
#define _LSEG_SBE_QTYENCODING_CXX_H_
...
public:
    ...
    QtyEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        QtyEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    QtyEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        QtyEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    QtyEncoding &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    {
        return static_cast<std::int8_t>(-2);
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, QtyEncoding &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
