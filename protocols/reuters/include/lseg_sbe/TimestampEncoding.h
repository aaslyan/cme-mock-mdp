...
#ifndef _LSEG_SBE_TIMESTAMPENCODING_CXX_H_
#define _LSEG_SBE_TIMESTAMPENCODING_CXX_H_
...
public:
    ...
    TimestampEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        TimestampEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    TimestampEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        TimestampEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    TimestampEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    TimestampEncoding &time(const std::uint64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, TimestampEncoding &writer)
{
    builder << '{';
    builder << R"("time": )";
    builder << +writer.time();
