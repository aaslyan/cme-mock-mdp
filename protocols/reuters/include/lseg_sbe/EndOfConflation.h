...
#ifndef _LSEG_SBE_ENDOFCONFLATION_CXX_H_
#define _LSEG_SBE_ENDOFCONFLATION_CXX_H_
...
public:
    ...
    EndOfConflation(char *buffer, const std::uint64_t bufferLength) :
        EndOfConflation(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    EndOfConflation(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        EndOfConflation(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    EndOfConflation &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    EndOfConflation &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        EndOfConflation skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    EndOfConflation &conflationID(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static const char *DecryptionKeyMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static const char *decryptionKeyCharacterEncoding() SBE_NOEXCEPT
    {
        return "null";
    }

    static SBE_CONSTEXPR std::uint64_t decryptionKeySinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    bool decryptionKeyInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    static SBE_CONSTEXPR std::uint16_t decryptionKeyId() SBE_NOEXCEPT
    {
        return 30784;
    }

    static SBE_CONSTEXPR std::uint64_t decryptionKeyHeaderLength() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD std::uint16_t decryptionKeyLength() const
    {
        std::uint16_t length;
    ...
    std::uint64_t skipDecryptionKey()
    {
    ...
    SBE_NODISCARD const char *decryptionKey()
    {
        std::uint16_t lengthFieldValue;
    ...
    std::uint64_t getDecryptionKey(char *dst, const std::uint64_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    EndOfConflation &putDecryptionKey(const char *src, const std::uint16_t length)
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
    ...
    std::string getDecryptionKeyAsString()
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    std::string getDecryptionKeyAsJsonEscapedString()
    {
        std::ostringstream oss;
        std::string s = getDecryptionKeyAsString();
    ...
    #if __cplusplus >= 201703L
    std::string_view getDecryptionKeyAsStringView()
    {
        std::uint64_t lengthOfLengthField = 2;
        std::uint64_t lengthPosition = sbePosition();
        sbePosition(lengthPosition + lengthOfLengthField);
    ...
    EndOfConflation &putDecryptionKey(const std::string &str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
        }
        return putDecryptionKey(str.data(), static_cast<std::uint16_t>(str.length()));
    }

    #if __cplusplus >= 201703L
    EndOfConflation &putDecryptionKey(const std::string_view str)
    {
        if (str.length() > 65534)
        {
            throw std::runtime_error("std::string too long for length type [E109]");
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const EndOfConflation &_writer)
{
    EndOfConflation writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "EndOfConflation", )";
    ...
    builder << ", ";
    builder << R"("DecryptionKey": )";
    builder << '"' <<
        writer.skipDecryptionKey() << " bytes of raw data\"";
    builder << '}';
...
SBE_NODISCARD static std::size_t computeLength(std::size_t decryptionKeyLength = 0)
{
...
#pragma GCC diagnostic ignored "-Wtype-limits"
    ...
    length += decryptionKeyHeaderLength();
    if (decryptionKeyLength > 65534LL)
    {
        throw std::runtime_error("decryptionKeyLength too long for length type [E109]");
    }
    length += decryptionKeyLength;
