...
#ifndef _LSEG_SBE_THRESHOLDENCODING_CXX_H_
#define _LSEG_SBE_THRESHOLDENCODING_CXX_H_
...
public:
    ...
    ThresholdEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        ThresholdEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    ThresholdEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        ThresholdEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    ThresholdEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    ThresholdEncoding &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, ThresholdEncoding &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
