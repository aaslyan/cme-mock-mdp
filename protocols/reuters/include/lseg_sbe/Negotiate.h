...
#ifndef _LSEG_SBE_NEGOTIATE_CXX_H_
#define _LSEG_SBE_NEGOTIATE_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(71);
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(500);
    ...
    {
        Negotiate skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    Negotiate &messageType(const MessageTypeEnum::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    Negotiate &sessionID(const std::uint64_t index, const char value)
    {
        if (index >= 20)
        {
    ...
    Negotiate &putSessionID(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    Negotiate &putSessionID(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    Negotiate &putSessionID(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    Negotiate &putSessionID(const char (&src)[N]) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_STRING_VIEW
    Negotiate &putSessionID(const std::string_view str)
    {
    ...
    #else
    Negotiate &putSessionID(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 20)
        {
...
public:
    ...
    Negotiate &flow(const FlowTypeEnum::Value value) SBE_NOEXCEPT
    {
        char val = (value);
    ...
    SBE_NODISCARD static const char *CredentialsMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::SEMANTIC_TYPE: return "Object";
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t credentialsSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool credentialsInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t credentialsEncodingOffset() SBE_NOEXCEPT
    {
        return 30;
    }
...
public:
    SBE_NODISCARD Credentials &credentials()
    {
        m_credentials.wrap(m_buffer, m_offset + 30, m_actingVersion, m_bufferLength);
        return m_credentials;
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const Negotiate &_writer)
{
    Negotiate writer(
        _writer.m_buffer,
        _writer.m_offset,
